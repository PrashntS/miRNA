...
Terrarium = (width, height, options) ->
  cellSize = undefined
...
Terrarium::makeGridWithDistribution = (distribution) ->
  current = undefined
  rand = 0
  grid = []
  x = 0
...
Terrarium::step = (steps) ->
  ...
  copyAndRemoveInner = (origCreature) ->
    if origCreature
      copy = _.assign(new (origCreature.constructor), origCreature)
