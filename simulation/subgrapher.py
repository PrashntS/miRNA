...
'''
def form_data_mirna():
	mirna_lis = random.sample(list(miRNA_meta_data.keys()), 10)
	loop = int(input('Enter number of miRNAs you wish to choose.\n'))
	for i in range(0,loop):
		print(mirna_lis)
		user_input = str(input('Choose miRNAs from above list.\n'))
		if user_input in mirna_lis:
			if user_input in list(miRNA_meta_data.keys()):
...
def collect_gene_info_for_mirna(user_input):
	...
	if 'Target Gene with Transcript Count' in list(miRNA_meta_data[user_input].keys()):
	...
	if 'Host Gene' in list(miRNA_meta_data[user_input].keys()) and not miRNA_meta_data[user_input]['Host Gene'] == '':
...
def mirna_subgraph(mirna_dict, gene_lis_for_mirna_subgraph):
	G = nx.DiGraph()
	G.add_nodes_from(list(mirna_dict.keys()))
...
'''
def form_data_genes():
	gene_lis = random.sample(list(gene_meta_data.keys()), 10)
	loop = int(input('Enter number of genes you wish to choose.\n'))
	for i in range(0,loop):
		print(gene_lis)
		user_input = str(input('Choose genes from above list.\n'))
		if user_input in gene_lis:
			if user_input in list(gene_meta_data.keys()):
...
def collect_mirna_info_for_gene(user_input):
	...
	if 'Host for' in list(gene_meta_data[user_input].keys()) and not gene_meta_data[user_input]['Host for'] == '':
		mirna_host_lis.append(gene_meta_data[user_input]['Host for'])
	if 'Target for' in list(gene_meta_data[user_input].keys()):
...
def gene_subgraph(gene_dict, mirna_lis_for_gene_subgraph):
	G = nx.DiGraph()
	G.add_nodes_from(list(gene_dict.keys()))
...
def both(input_mirna, input_gene):
	for mirna in list(miRNA_meta_data.keys()):
		'''
		Creates a list of genes to be included as nodes.
		Creates miRNA dictionary with host gene and target genes with affinity.
		'''
		...
		if input_mirna == mirna:
			...
			if 'Target Gene with Transcript Count' in list(miRNA_meta_data[mirna].keys()):
				target_gene_lis_for_mirna_subgraph = miRNA_meta_data[input_mirna]["Target Gene with Transcript Count"]

			if 'Host Gene' in list(miRNA_meta_data[input_mirna].keys()) and not miRNA_meta_data[input_mirna]['Host Gene'] == '':
	...
	for gene in list(gene_meta_data.keys()):
		'''
		Creates a list of miRNAs to be included as nodes.
		Creates gene dictionary with 'host for' and 'target for' keys with miRNA as values.
		'''
		...
		mirna_target_lis = []
		if input_gene == gene:
			if 'Host for' in list(gene_meta_data[input_gene].keys()) and not gene_meta_data[input_gene]['Host for'] == '':
				mirna_host_lis.append(gene_meta_data[input_gene]['Host for'])
			if 'Target for' in list(gene_meta_data[input_gene].keys()):
